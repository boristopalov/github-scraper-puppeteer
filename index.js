...
const { searchCommitsForEmail } = require('./api');
...
const scrape = async (url, callback) => {
    ...
    while (true) {
        ...
        for await (const user of users) { 
            ...
            const email = await searchCommitsForEmail(usernameText, nameText);
            if (!email) { 
                break scrape;
            };
            ...
            const isInNewYork = searchTextForKeywords(locationText, ["new york", "ny"]) && !searchTextForKeywords(locationText, ["germany"]);
            if (isInNewYork) {
                // scrape the user's profile
                const url = await (await (await user.$('a.d-inline-block.no-underline.mb-1')).getProperty('href')).jsonValue();                
                await scrapeUser(url);
            };
